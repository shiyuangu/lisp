...
(defun gtags-goto-tag (tagname flag &optional other-win)
  (let (option context save prefix buffer lines flag-char)
    ...
    (setq option "-Tx")   ;;hack by sgu: always search throughs all paths in GTAGSLIBPATH
    (if (gtags-ignore-casep)
        (setq option (concat option "i")))
    ...
    (let (status)
      ...
      (setq status (if (equal flag "C")
		      ;;hack of sgu: display the commands   
		      (progn
			  (message (concat "Running command:"  gtags-global-command " " option "--encode-path=\" \t\"" " " context " " tagname))
                          (call-process gtags-global-command nil t nil option "--encode-path=\" \t\"" context tagname))
		      (progn
			  (message (concat "Running command:" gtags-global-command " " option "--encode-path=\" \t\"" " " tagname))
